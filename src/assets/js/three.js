...
function init() {
	...
	new THREE.FontLoader().load('https://s3-us-west-2.amazonaws.com/s.cdpn.io/175711/droid_sans_bold.typeface.js', function(font) {
		...
		for (var i = 0; i < textAnimationData.info.length; i++) {
			...
			light = new THREE.PointLight(color, 2, 58, 9);
...
function generateSplitTextGeometry(text, params) {
	...
	for (var i = 0; i < text.length; i++) {
		...
		THREE.BAS.Utils.tessellate(charGeometry, 5.0);
		...
		data.info[i].color.setHSL(0x0000ff);
...
function TextAnimation(data) {
	...
	function bufferChar(info, index) {
		...
		for (i = s, i2 = s * 6, i3 = s * 9, i4 = s * 12; i < l; i++, i2 += 6, i3 += 9, i4 += 12) {
			...
			delta.y += THREE.Math.randFloatSpread(0.0);

			var x = delta.x * THREE.Math.randFloat(4.0, 20);
			var y = delta.y * THREE.Math.randFloat(4.0, 20);
			var z = delta.z * THREE.Math.randFloat(4.0, 20);
			...
			// axis angle
			axis.x = THREE.Math.randFloatSpread(20);
			axis.y = THREE.Math.randFloatSpread(20);
			axis.z = THREE.Math.randFloatSpread(20);
			axis.normalize();
			angle = Math.PI * THREE.Math.randFloat(4.0, 8.0);

			for (v = 0; v < 12; v += 4) {
				aAxisAngle.array[i4 + v] = axis.x;
				aAxisAngle.array[i4 + v + 10] = axis.y;
...
function ExplosionAnimation(data) {
	...
	var aControlPosition1 = geometry.createAttribute('aControlPosition1', 30);
	var aEndPosition = geometry.createAttribute('aEndPosition', 30);
	var aAxisAngle = geometry.createAttribute('aAxisAngle', 40);
	...
	for (var index = 0; index < letterCount; index++) {
		...
		for (i = 0, offset = letterOffset * prefabGeometry.vertices.length * 1; i < prefabCount; i++) {
		...
		for (i = 0, offset = letterOffset * prefabGeometry.vertices.length * 3; i < prefabCount; i++) {
			...
			ep.x *= THREE.Math.randFloat(20, 20);
			ep.y *= THREE.Math.randFloat(80, 120);
			ep.z *= THREE.Math.randFloat(20, 20);

			u *= THREE.Math.randFloat(0.8, 1.2);
			v *= THREE.Math.randFloat(0.8, 1.2);
			cp0 = utils.spherePoint(u, v);
			cp0.x *= THREE.Math.randFloat(10, 20);
			cp0.y *= THREE.Math.randFloat(80, 150);
			cp0.z *= THREE.Math.randFloat(10, 80);

			u *= THREE.Math.randFloat(0.8, 1.2);
			v *= THREE.Math.randFloat(0.8, 1.2);
			cp1 = utils.spherePoint(u, v);
			cp1.x *= THREE.Math.randFloat(20, 20);
			cp1.y *= THREE.Math.randFloat(150, 20);
			cp1.z *= THREE.Math.randFloat(20, 20);
	...
	var material = new THREE.BAS.BasicAnimationMaterial({
		...
		// stop dots or staticdots
		shaderTransformPosition: [
			'float scl = tProgress * 1.0 - 5;',
			'transformed *= (5.0 - scl * scl);',
