...
export class ProfileSettingsComponent implements OnInit {
  ...
  profile_details = {
    ...
    identity_verification_validation : "",

    identity_verification_name : "Choose..",
    identity_verification_name_validation : "",

    web_address : "",
    web_address_validation : "",
    
    profile_headlines : "",
    profile_headlines_validation : "",
  }
  ...
  ngOnInit(): void {
    ...
    const reg = '(^(http[s]?://)?([w]{1}[.])?([a-z0-9]+[.])+(com|org|co.in|in|io)(((/[a-z0-9]+)*(/[a-z0-9]+/))*([a-z0-9]+[.](html|php|gif|png))?)$)|(^([.]/)?((([a-z0-9]+)/?)+|(([a-z0-9]+)/)+([a-z0-9]+[.](html|php|gif|png)))?$)';

    // validate
    this.projectForm = new FormGroup({
      project_name: new FormControl('', [Validators.required]),    
      project_URL: new FormControl('', [Validators.pattern(reg)]),    
    })
    ...
    this.publicationForm = new FormGroup({
      ...
      publication_url : new FormControl('', [Validators.pattern(reg)]),    
    })
  ...
  getUserProfile(){
    ...
    this.service.getUserProfile().subscribe(async res => {
      ...
      console.log(' res.profile.talent_accomplishment.languages', res.profile.talent_accomplishment.languages)
      ...
      if (res.profile && res.profile.talent_category_data && res.profile.talent_category_data.discipline_id) {
        ...
        this.dId=res.profile.talent_category_data.discipline_id;
      ...
      if (res.profile && res.profile.talent_category_data && res.profile.talent_category_data.speciality.length) {
              if(this.selected_discipline== res.profile.talent_category_data.discipline_data.id){
      ...
      if (res.profile && res.profile.talent_accomplishment && res.profile.talent_accomplishment.languages && res.profile.talent_accomplishment.languages.languages ) {
        this.languagesModel.other_language = res.profile.talent_accomplishment.languages.languages;
      } 

      if (res.profile && res.profile.talent_accomplishment && res.profile.talent_accomplishment.languages && res.profile.talent_accomplishment.languages) {
        this.languagesModel.english_proficiency = res.profile.talent_accomplishment.languages.english_proficiency;
              console.log('this.languagesModel.english_proficiency',this.languagesModel.english_proficiency)

      } 
      if(this.selected_discipline== res.profile.talent_category_data.discipline_id){
  ...
  selectDiscipline(discipline){
    ...
    console.log('selected_discipline',this.selected_discipline,this.dId)
  ...
  getDisciplineSpeciality(){
   // alert(this.selected_specialities.length)
    const discipline_id = this.selected_discipline;
   
    console.log('this.selected_specialities',this.selected_specialities)
  ...
  getDisciplineServices(selected_specialities){
    ...
    this.service.getDisciplineServicestalent(this.selected_specialities).subscribe(async res => {
  ...
  addNewSpeciality(){
    if(this.serviceName && this.serviceName.trim()){
      ...
      this.service.addSpecialityServicetalent(obj).subscribe(async res => {
  ...
  onSelect(evt,licenses) {
    ...
    if(true)
    {
        ...
        this.service.uploadMedia(formData).subscribe(async res => {
          ...
          this.certificates.documents.push(obj);
          this.isLoading = false;
        },(err) => {
          this.service.handleError(err)
          console.log(err)
  ...
  console.log(index)
            console.log("lucense",this.licenses_list)

    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'License'?");
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
        if(dialogResult){
          ...
          this.profile.talent_qualification.licenses.splice(index,1)
          console.log(this.profile.talent_qualification.licenses)
  ...
  removeExperience(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Experience'?");
  ...
  // }
  getUniversities(i){
    if(this.education.school_name){
      this.service.getUniversities(this.education.school_name).subscribe(async res => {
  ...
  getUniversityText(name,i){
    this.education.school_name = name;
    this.university_list = [];
  }
  ...
  getDegreeText(name,i){
    this.education.degree = name;
    this.degree_list = [];
  }

  getFieldOfStudies(i){
    if(this.education.field_of_study){
      const obj = {
        search:this.education.field_of_study
      }
  ...
  getFieldOfStudyText(name,i){
    this.education.field_of_study = name;
    this.field_of_study_list = [];
  }

  getLicenses(i){
    
    if(this.certificates.certificate_name && this.certificates.certificate_name.trim() && this.speciality_id){
      const obj = {
        speciality_id:this.speciality_id,
        search:this.certificates.certificate_name
      }
  ...
  getLicenseText(name,i){
    this.certificates.certificate_name = name;
    this.license_list = [];
  }
  ...
  changeExpectedGraduation(event,ind){
    ...
    if(event.target.checked){
      ...
      this.education.expectedGraduation = true;

      this.education_form.controls['end_date'].setValidators([]);
      this.education_form.controls['end_date'].updateValueAndValidity();

    }else{
      this.today = new Date(); 
      this.today1 = new Date(); 
      this.minDate1 = new Date(this.currentYear, 0, 1);
      this.education.expectedGraduation = false;

      this.education_form.controls['end_date'].setValidators([Validators.required]);
      this.education_form.controls['end_date'].updateValueAndValidity();
    }
  }
  ...
  addLanguage(){
    ...
    if(this.languagesModel.other_language && this.languagesModel.other_language.length){
      ...
      type : "languages",
      record : this.languagesModel.other_language
    }).subscribe(resp => {    
  ...
  validateExp(){
    ...
    if(this.education_list && this.education_list.length){
      this.education_list.map((item,i) => {
        ...
        }else if(i==this.education_list.length-1){
    ...
    if(this.licenses_list && this.licenses_list.length){
      this.licenses_list.map((item,i) => {
        ...
        if(item.certificate_name && 
          ...
          item.certificationUrlValidation==false && 
          ((
            item.is_license_expire==false && 
            item.endYearValidation==false && 
            item.endMonthValidation==false) || 
            (item.is_license_expire==true)) && 
            i==this.licenses_list.length-1)
          {
          if(!flag){
            isValid = true
          }
        }else if(i==this.licenses_list.length-1){
  ...
  checkEducationData(){
    ...
    const isValid = this.validateLic();
    const isValidExp = this.validateExp();
  ...
  removeSkills(_id){
    this.selected_skills.splice(this.selected_skills.findIndex(ele=>ele === _id),1);
  ...
  removeIndustry(_id){
    this.selected_industry.splice(this.selected_industry.findIndex(ele=>ele === _id),1);
  }
  ...
  removeTools(_id){
    this.selected_tools.splice(this.selected_tools.findIndex(ele=>ele === _id),1);
  }
  ...
  removeInterSkills(_id){
    this.selected_interpersonal_skill.splice(this.selected_interpersonal_skill.findIndex(ele=>ele === _id),1);
  }
  ...
  addIndustry(speciality_id){
    ...
    if(industry_name.length==1){
      ...
      this.service.addSkill(obj).subscribe(async res => {
  ...
  get project_URL(): any {
    return this.projectForm.get('project_URL');
  }
  ...
  getProfile(){
    ...
    this.service.getUserProfile().subscribe(res => {
      ...
      this.selected_interpersonal_skill=[]
      }
      ...
      if(this.profile && this.profile.portfolio){
        this.profile_details.portfolio = this.profile.portfolio;
      }

      if(this.profile && this.profile.identity_document){
        this.profile_details.identity_verification_name = this.profile.identity_document;
      }

      if(this.profile && this.profile.web_address){
        this.profile_details.web_address = this.profile.web_address;
      }

      if(this.profile && this.profile.profile_headline){
        this.profile_details.profile_headlines = this.profile.profile_headline;
      }
  ...
  hideShow(type1,type2){    
    ...
    if(type1 == "education"){
      ...
      this.education_form.markAsUntouched()
  ...
  editWork(index){
    ...
    if(data){
      ...
      this.work_history.location = data.location;
      if(data.media){
  ...
  updateWork(){
    ...
    if(this.work_history.start_year && this.work_history.end_year && this.work_history.start_year > this.work_history.end_year){
      this.service.showErrorMessage({
          message : "End year must be greater than start year"
      });
      return false
    }
     if(this.work_history.start_month && this.work_history.end_month && this.work_history.start_month > this.work_history.end_month){
      this.service.showErrorMessage({
          message : "End year must be greater than start year"
      });
      return false
  ...
  deleteWork(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Work'?");
  ...
  editOrganization(index){
    ...
    if(data){
      ...
      this.organization.start_date = data.start_date;
      this.organization.end_date = data.end_date;
  ...
  editEducation(index){
    const data = this.profile.talent_qualification.education[index];
    if(data){
      this.education.school_name = data.school_name;
      this.education.degree = data.degree;
      this.education.field_of_study = data.field_of_study;
      this.education.start_date = moment(data.start_date).format("YYYY-MM-DD");
      this.education.end_date = moment(data.end_date).format("YYYY-MM-DD");
      this.education.grade = data.grade;
      this.education.activities = data.activities;
      this.education.expectedGraduation = data.expectedGraduation;
      this.education.index = index;
      this.education.documents = data.documents;
  ...
  editCertificate(index){
    const data = this.profile.talent_qualification.licenses[index];
    if(data){
      this.certificates.certificate_name = data.certificate_name;
      this.certificates.certification_authority = data.certification_authority;
      this.certificates.license_no = data.license_no;
      this.certificates.start_month = data.start_month
      this.certificates.end_month = data.end_month
      this.certificates.start_year = data.start_year
      this.certificates.end_year = data.end_year
      this.certificates.certification_url = data.certification_url;
      this.certificates.index = index;
      this.certificates.is_license_expire = data.is_license_expire;
      this.certificates.documents = data.documents;
  ...
  updateEducationData(){
    ...
    this.service.editMyProfileQualification({
      type : "education",
      record : this.profile.talent_qualification.education
    }).subscribe(resp => {    
  ...
  console.log(this.profile.talent_qualification.licenses)
    this.service.editMyProfileQualification({
      type : "licenses",
      record : this.profile.talent_qualification.licenses
    }).subscribe(resp => {    
 ...
 let record={"languages": this.languagesModel.other_language , "english_proficiency":this.languagesModel.english_proficiency}
    this.service.editMyProfileAccomplishments({
      type : "languages",
      record :  record
  ...
  updateOrganization(){
    ...
    if(this.organization.start_date >= this.organization.end_date){
      this.service.showErrorMessage({
          message : "End year must be greater than start year"
      });
      return false
  ...
  updateEducation(){
    const index = parseInt(this.education.index);
    delete this.education.index;
    
    if(this.education.start_date && this.education.end_date && this.education.start_date >= this.education.end_date){
      this.service.showErrorMessage({
    ...
    if(index < 0 || isNaN(index)){ 
      if(!this.profile.talent_qualification.education){
        this.profile.talent_qualification.education = []
      }
      this.profile.talent_qualification.education.push(this.education)
    }else{
      console.log(this.profile.talent_qualification.education);
      this.profile.talent_qualification.education[index] = this.education;
  ...
  updateCertificate(){
    const index = parseInt(this.certificates.index);
    delete this.certificates.index;
    if(this.certificates.certification_url){
      if(!this.validURL(this.certificates.certification_url.trim())){
        this.profile_details.web_address_validation = "Invalid URL"
        return false
      }else{
        this.profile_details.web_address_validation = ""
      }
    }
    
    if(this.certificates.start_year && this.certificates.end_year && this.certificates.start_year >= this.certificates.end_year){
      this.service.showErrorMessage({
          message : "End year must be greater than start year"
      });
      return false
    ...
    if(index < 0 || isNaN(index)){ 
      if(!this.profile.talent_qualification.licenses){
        this.profile.talent_qualification.licenses = []
      }
      this.profile.talent_qualification.licenses.push(this.certificates)
    }else{
      console.log(this.profile.talent_qualification.licenses);
      this.profile.talent_qualification.licenses[index] = this.certificates;
  ...
  deleteOrganization(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Organization'?");
  ...
  deleteEducation(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Education'?");
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
        if(dialogResult){
          ...
          this.profile.talent_qualification.education.splice(index,1)
          this.updateEducationData();
  ...
  deleteCourse(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Course'?");
  ...
  editProject(index){
    ...
    if(data){
      ...
      this.project.start_date = data.start_month;
      this.project.end_date = data.start_year;
  ...
  updateProject(){
    ...
    console.log("this project ============ ",this.project)
     if(this.project.project_url){
      if(!this.validURL(this.project.project_url.trim())){
        this.profile_details.web_address_validation = "Invalid URL"
        return false
      }else{
        this.profile_details.web_address_validation = ""
  ...
  deleteProject(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Project'?");
  ...
  addMember(pubIndex){
    const obj= {name:''};
    this.project.team_members.unshift(obj);
  ...
  uploadEducation(evt: any) {
    ...
    if(true)
    {
        ...
        this.service.uploadMedia(formData).subscribe(async res => {
          ...
          console.log("upload media obj ============= ",obj)
          this.education.documents.push(obj);
          this.isLoading = false;
        },(err) => {
          this.service.handleError(err)
          console.log(err)
  ...
  deleteEducatonDoc(ind){
    const document = this.education.documents[ind]

    console.log(document)
    console.log(this.education.documents)

    this.service.deleteFile(document.url ? document.url : document.document).subscribe(async res => {
      this.education.documents.splice(ind,1);
  ...
  deleteDocument(document,certificate_array_index,document_array_index){
    // this.isLoading = true;
    console.log("document",document)
    console.log("certificate_array_index",certificate_array_index)
    console.log("document_array_index",document_array_index)
    this.service.deleteFile(document.url ? document.url : document.document).subscribe(async res => {
      this.certificates.documents.splice(document_array_index,1)
      this.isLoading = false;
    },(err) => {
      this.service.handleError(err)
      this.isLoading = false;
  ...
  deleteAward(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Reward'?");
  ...
  deleteRecommend(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Recommendations'?");
  ...
  updatePublication(){
    ...
    if(this.publication.publication_url){
      if(!this.validURL(this.publication.publication_url.trim())){
        this.profile_details.web_address_validation = "Invalid URL"
        return false
      }else{
        this.profile_details.web_address_validation = ""
  ...
  deletePublication(index){
    ...
    const dialogData = new ConfirmDialogModel("Confirm Action", "Are you sure to delete 'Publication'?");
  ...
  addAuthor(pubIndex){
    const obj= {value:''};
    this.publication.authors.unshift(obj);
  ...
  onSelectUserId(event) {
    if (event.target.files && event.target.files[0]) {
      ...
      if(file.type.split('/')[0] != "image"){ // make sure video file selected
      ...
      console.log(this.single_fields.video_introduction)
  ...
  onSelectIdentity(event) {
    if (event.target.files && event.target.files[0]) {
      ...
      this.profile_details.identity_verification = file.name
            console.log(this.single_fields.video_introduction)

      // this.profileDetailModel.intro_video = file
      this.profile_details.identity_verification = file
    }
  }
  ...
  updateSector(){
    ...
    const slecRegion = this.sector_list.filter(ele=> ele['flag'] === true);
    
    const sl = slecRegion.map(i => i._id)
    ...
    this.hideShow('sector','')

    //console.log(slecRegion)
    // return
    this.service.editSector({
      type : "discipline_sectors",
      record : sl
    }).subscribe(resp => {   

      this.hidden_blocks.sector = "";
      this.service.showSuccessMessage({
  ...
  getServicenew(){
    ...
    const trad = this.traditional_data.map(item => {
      const services = item.services.filter(serv => {
        return serv.flag
      })
      return services.map(it => it._id);
    })

    const digi = this.digital_data.map(item => {
      const services = item.services.filter(serv => {
        return serv.flag
      })
      return services.map(it => it._id);
    ...
    var mergedTrad = [].concat.apply([], trad);
    var mergedDigital = [].concat.apply([], digi);
  ...
  getSelectedSkillNew(){
    ...
    const toolsSkill = this.tools_list.map(item => {
      const services = item.skills.filter(serv => {
        return serv.flag
      })
      return services.map(it => it._id);
    })

    const industrySkill = this.industry_list.map(item => {
      const services = item.skills.filter(serv => {
        return serv.flag
      })
      return services.map(it => it._id);
    ...
    const interpersonalSkill = this.interpersonal_skill_list.map(item => {
      const services = item.skills.filter(serv => {
        return serv.flag
      })
      return services.map(it => it._id);
    ...
    var mergedTool = [].concat.apply([], toolsSkill);
    var mergedIndustry = [].concat.apply([], industrySkill);
    var mergedInterpersonal = [].concat.apply([], interpersonalSkill);
  ...
  sendRequestToAdmin(){
   // console.log(this.getSelectedSkillNew())

    // if(this.profile && this.profile.profile_request && this.profile.profile_request.profile_request_submitted_to_admin){
    //   this.service.showErrorMessage({
    //     message : "Please wait until Admin review your request."
    //   })
    //   return;
    // }
    
    // validate all data before send request
    this.getProfile();
    this.getUserProfile();
    console.log(this.selected_specialities)
    if(!this.selected_discipline){
      this.service.showErrorMessage({
        message : "Please select Descipline "
      })
    }  else if(!(this.selected_specialities.length)){
      this.service.showErrorMessage({
        message : "Please select atleast one Speciality"
      })
    } else if(!this.getServicenew().mergedDigital.length && !this.getServicenew().mergedTrad.length ){
      this.service.showErrorMessage({
        message : "Please select atleast one Service"
      })
    }else if(!this.selected_industry.length){
      this.service.showErrorMessage({
        message : "Please select atleast one Industory skill"
      })
    } else if(!this.selected_tools.length){
      this.service.showErrorMessage({
        message : "Please select atleast one Tools skill"
      })
    } else if(!this.selected_interpersonal_skill.length){
      this.service.showErrorMessage({
        message : "Please select atleast one Interpersonal skill"
      })
    } else {
      this.isLoadingNew = true;
      const form = {
        discipline : this.selected_discipline,
        specialities : this.selected_specialities,
        traditional_service : this.getServicenew().mergedTrad,
        digital_service : this.getServicenew().mergedDigital,
        technologies_skills : this.selected_tools,
        interpersonal_skills  :this.selected_interpersonal_skill,
        industry_knowledge_skills : this.selected_industry
      }
      this.service.sendProfileRequest(form).subscribe(resp => {
  ...
  preventSpace(event){
   ...
   saveProfileDetails(){
    ...
    // verify Web address
    if(this.profile_details.web_address){
      if(!this.validURL(this.profile_details.web_address.trim())){
        this.profile_details.web_address_validation = "Invalid URL"
        return false
      }else{
        this.profile_details.web_address_validation = ""
    ...
    // Verifiy Portfolio 
    if(this.profile_details.portfolio){
      if(!this.validURL(this.profile_details.portfolio.trim())){
        this.profile_details.portfolio_validation = "Invalid URL"
        return false
      }else{
        this.profile_details.portfolio_validation = ""
      }
    }

    const form = new FormData();
    if(this.profile_details.identity_verification){
      form.append('userID',this.profile_details.identity_verification); 

    }
    form.append('portfolio',this.profile_details.portfolio); 
    form.append('web_address',this.profile_details.web_address); 
    form.append('profile_headline',this.profile_details.profile_headlines); 
  ...
  compareProject(){
     if(new Date(this.project.start_date) >= new Date(this.project.end_date)){
      this.error={isError:true,errorMessage:'Start date can not be greater than End date '};
  ...
  compareorganization(){
    console.log("test")
   if(new Date(this.organization.start_date) >= new Date(this.organization.end_date)){
      this.error={isError:true,errorMessage:'Start date can not be greater than End date '};
      console.log(this.error)
   }else{
   this.error={isError:false};
  ...
  uploadProfileImage(evt: any) {
    ...
    if (type == "image") {
        ...
        this.service.saveProfileDetails(formData).subscribe(async res => {
          this.service.showSuccessMessage({
            message : "Profile image updated"
          })

          const p = document.getElementById('pro-img-header')
          console.log(p);
          if(p){
            p.setAttribute('src',res.flag.image)
          }
          // res.profile.image
          this.isLoading = false;
          this.getProfile();
